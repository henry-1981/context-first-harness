---
name: check-harness-context-first
description: |
  **비개발자 하네스 파생** — 원본 check-harness v3(24항목)에서 개발 지향 판정 기준 3건(B1·D1·D2) 재정의 + 공백 영역 6건(refs INDEX·wiki 5-layer·skill+wiki 페어·출력 스킬·handoff 라이프사이클·seCall→codex 파이프라인) 신규 추가한 **6축 30항목 체크리스트**.
  검증 기준이 "test runner/포맷터 훅"이 아니라 "deliverable-review/wiki-check/wrap" 같은 문서 산출물 게이트. 맥락 축은 refs/wiki/skill+wiki 페어 무결성까지 본다.
  4개 서브에이전트(skill-portfolio-analyzer, session-pattern-analyzer, context-quality-reviewer, **context-first-auditor**)를 병렬 실행.
  session-pattern-analyzer는 User 전역과 현재 프로젝트 두 번 돌려 User/Project 스코프를 분리한다.

  Use when auditing a **context-first (non-dev) harness** — a repo where the primary artifacts are prose documents, rules, wiki pages, and domain thinking frames rather than source code. Prefer this over `check-harness` when the project has refs/, wiki/, handoff lifecycle, or skill+wiki pairs.

  Trigger: "/check-harness-context-first", "context-first 진단", "비개발자 하네스 점검", "문서 하네스 진단", "context harness audit".
allowed-tools:
  - Read
  - Grep
  - Glob
...
<!--
Source: https://github.com/team-attention/harness (harness-session plugin v0.3.1) — 원본 check-harness
Imported: 2026-04-17. 비개발자 하네스 파생으로 2026-04-19 분기.
변경: B1·D1·D2 재정의, C7-C9·B7·E4-E5 신규 추가, project-automation-auditor → context-first-auditor 교체.
Upstream license: none specified at import time. Reused with attribution.
-->

# /check-harness-context-first — Context-First Harness 성숙도 진단 (v4)

**6축 사이클** 로 평가한다: **구조 → 맥락 → 계획 → 실행 → 검증 → 개선**.
체크리스트 원본: `references/checklist.md` (**30항목 — 반드시 먼저 읽어 v4 항목 정의 확인**).
...
Agent(
  subagent_type: "context-first-auditor",
  description: "Context-first automation, verification & compounding audit",
  prompt: """
    project_root={PROJECT_ROOT}
    session_report=/tmp/cc-cache/check-harness/SESSION_PROJECT.json  (있으면 참조)
    비개발자 하네스 판정 기준으로 감사 — D1/D2/B1 재정의(deliverable-review·wiki-check·wrap 등가물) + 신규 6항목(refs INDEX·wiki 5-layer·skill+wiki 페어·출력 스킬·handoff 라이프사이클·seCall→codex 파이프라인) 수집 + compounding 신호.
...
| **2. 맥락** | CONTEXT + AUTOMATION | Project | claude_md·rules·sensitive (C1–C6) + refs INDEX·wiki 5-layer·skill+wiki 페어 (C7–C9, AUTOMATION에서) |
| **3. 계획** | SESSION_USER | User | plan_first_ratio (B2) |
| **4. 실행** | SESSION_USER + SESSION_PROJECT + AUTOMATION | User+Project | delegation/parallel/top_ngram (B3·B5·B6) + output_skills_invoked (B7, AUTOMATION) — **두 스코프 각각 계산** |
| **5. 검증** | SESSION_PROJECT + AUTOMATION | Project | 재정의 B1(검증성 스킬 호출 ratio) + D1(deliverable-review 자산) + D2(문서 린트 훅) + D3–D5 |
| **6. 개선** | AUTOMATION.compounding + SESSION (wrap/memory) | Both | E1·B4·E2·E3 + E4(handoff 라이프사이클) + E5(seCall→codex 최근성) |
