...
description: "Analyzes installed skills/plugins/MCP servers against ~/.claude.json to detect dead, ghost, duplicate, and collision-prone entries and report the current accessible state (enabled plugins, runtime skills, connected MCP servers). Returns structured JSON. Read-only. Use from /check-harness."
