...
plan 단계에서 확정된 mode/direction/outline으로 풀 덱을 생성하고, deck-verifier로 검증하며, slides-grab 에디터로 사용자 미세 수정을 수용한다. 본 스킬이 완료되면 `design.status = "passed"`로 전이되어 `presentation:export`가 이어받는다.
...
| 1 → 2 | 동일하게 2차 재시도 |
| 2 → 3차 FAIL | Root cause 리포트 생성(Spec §5.2 5옵션 제시) → 사용자 선택 gate. state.json `history`에 `root_cause_report` 이벤트 append |
...
- Y: 에디터 기동 후 사용자 "완료" 선언 시 design.status = "passed" 전이 (P7 진입)
...
**🚧 GATE: P7 진입 시 사용자 "완료" 선언 없이 design.status = "passed" 전이 금지.** (P6-B에서 에디터 skip한 케이스는 이미 P6-B에서 전이 완료. P7은 진입하지 않음.)
