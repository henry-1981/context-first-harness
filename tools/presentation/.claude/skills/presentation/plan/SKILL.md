...
**Main thread에서 직접 실행.** 서브에이전트 `Agent()` 호출은 내부 producer(storyteller)·verifier(outline-verifier, sample-verifier) 디스패치에만 사용한다. HB gate 상호작용(P2 outline 승인, P4 A/B 선택)이 본 스킬 내에 2회 있으므로 메인에서 돌려야 한다.
...
| 2건 이상 | HB에게 프로젝트 목록 제시 후 선택받기 |

`plan.status` 분기 (기존 프로젝트):

| plan.status | 처리 |
|---|---|
| `"passed"` | 내용 수정 모드. outline.md를 HB에게 보여주고 수정 지시 수신 → P2 outline-verifier 재실행부터 진행. A/B 샘플은 skip(mode·direction 이미 확정됨) |
| `"failed"` | 마지막 실패 지점부터 재개. `history` 배열 말미의 `*_verifier_fail` 이벤트로 위치 판정 |
| `"ready"` / 미기재 | 처음부터 진행 |

state.json 부재 + `draft/outline.md`·`draft/slides/slide-*.html` 등 산출물 파일만 존재하는 레거시 프로젝트는 Spec §8.1에 따라 HB에게 2옵션 제시: (1) state.json 수동 재구성(~5분 인터뷰) (2) 새 프로젝트 시작. 자동 마이그 없음.
...
HB에게 수집: 소스(파일/URL/주제), 목적, 분량, 청중, 이미지 유무, 섹션 인디케이터(A/B/C, 기본 C), **출력 모드**(PPTX/WebPPT, 기본 PPTX), **Aesthetic Direction**(WebPPT 또는 PPTX 자유 모드).
...
sm.merge('{state_path}', {
  source: {
    ...
    classification: '<prepared|raw|topic>'  // storyteller S4 대응. HB 선언 또는 간단 heuristic
  },
  config: {
    mode: '<pptx|webppt>',
...
`source.classification`은 HB에게 명시 질문: "이 소스는 (1) 이미 발표용으로 가공 완료 / (2) raw 자료 / (3) 주제 문자열만 중 어느 쪽인가?" 답을 `prepared|raw|topic`으로 저장. storyteller가 이 값을 읽어 편집자/구조화/작가 모드 분기.
...
FAIL 시 재시도 상한 2회(Spec §5.2). 3차 FAIL이면 root cause 리포트 생성 후 HB 선택 gate.

### P2-3. HB outline 승인 gate

outline.md 요약을 HB에게 제시(슬라이드 수·제목·섹션 구성). **🚧 GATE: HB 승인 없이 P3 진입 금지.** 수정 지시 있으면 storyteller 재호출.
...
**WebPPT 모드:**
- `webppt-free` × 2 병렬. Aesthetic Direction A/B 비교(HB가 direction 1개만 지정했으면 LLM이 소재 적합 대안 1개 추천해 A/B 구성)
...
**🚧 GATE: HB 선택 전까지 plan.status = "passed" 전이 금지.**
...
plan.status = "passed" 기록 직후 HB에게 통보:
...
다만 P4 gate(A/B 선택)·P2 gate(outline 승인)은 HB 응답 없이 진행 금지. gate 위반은 합리화 방어 표(기존 orchestrator 승계) 적용.

## 에러 핸들링

| 상황 | 대응 |
|---|---|
| source 파일 없음 | HB에게 경로 재확인 요청 |
| storyteller 2회 실패 | 3차 FAIL root cause 리포트(Spec §5.2) 생성 후 HB 선택 gate |
| sample 초과 생성 | 초과분 shell `rm` 후 sample-verifier 재호출 1회 |
| ab_compare 브라우저 오픈 실패 | `start ""` 재시도 1회 → 그래도 실패 시 HB에 경로 안내 후 수동 오픈 요청 |
| state.json schema validation 실패 | 해당 merge 롤백 + `history`에 `validation_fail` 기록 + HB 통보 |
...
| P4에서 "전체 테스트" 또는 샘플 수량 선택지 HB 제시 | 즉시 삭제, A/B 2 샘플 고정 |
