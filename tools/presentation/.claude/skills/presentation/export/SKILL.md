...
**Main thread 직접 실행.** 변환 command 실행 + export-verifier 서브에이전트 디스패치. 사용자 상호작용은 포맷 선택 1회.
...
| 상황 | 처리 |
|---|---|
| state.json 없음 또는 `design.status != "passed"` | **즉시 중단**. 사용자에 "design 단계를 먼저 완료하세요" 안내, `presentation:design` 호출 지시 |
...
FAIL 시 재시도 상한 2회. 3차 FAIL은 root cause 리포트(Spec §5.2, design 단계와 동일 5옵션 구조) → 사용자 gate.
...
| 상황 | 대응 |
|---|---|
| 진입 시 `design.status != "passed"` | 즉시 중단, 사용자에 design 선행 안내 |
| 변환 command 실패(예: build-webppt.py 에러) | stderr 캡처 + state.json `history`에 `export_convert_fail` + 사용자 통보 + 재시도 1회 |
| export-verifier 3차 FAIL | Root cause 리포트 + 사용자 선택 gate |
| 결과 파일 자동 오픈 실패 | 경로만 사용자에 안내 후 수동 오픈 요청 |
