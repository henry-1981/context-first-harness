...
Agent는 브라우저를 오픈하거나 slides-grab 에디터를 실행하지 않는다. Playwright 렌더링 캡처는 `visual-designer`의 Self-Check 프로토콜에 한해 허용되며, 그 외의 브라우저·에디터 실행 권한은 오케스트레이터(`presentation:run` 또는 HB 직접 호출 skill)에 집중된다.

근거: `lessons.md:1070-1072` (visual-designer가 슬라이드마다 `start ""` 실행하여 에디터 4회 중복 기동).
...
근거: `lessons.md:1062-1064` (storyteller paraphrase), `lessons.md:734-737` (Research Agent가 가공 콘텐츠에 raw 프레임 투영).
...
- 금지 어휘: 박제, 환류, 지렛대, 이주 (HB 지정)

상세 목록은 `Project/rules/content-writing.md` 참조(모노레포 전역 규칙). 본 §공통 규율은 presentation 도메인 한정 요약.

### 규율 4: 후처리 불가

생성된 HTML/CSS의 폰트·레이아웃·색상 후처리 패칭 금지. "생성 시점에 올바르게" 원칙. sed/python으로 HTML 일괄 수정 불가. 실패 시 해당 slide를 재생성한다.

근거: `lessons.md:714-721` (폰트 후처리 스케일링이 캔버스 넘침 유발).
