...
## 왜 3축인가 — 비개발자 작업 사이클의 자연 분할입니다

3축은 비개발자 작업 사이클의 세 단계에 그대로 대응합니다.

1) 도메인 지식을 AI가 인식할 수 있는 형태로 전달합니다 — 입력
2) AI가 처리한 결과를 사람이 받을 수 있는 형식으로 바꿉니다 — 출력
3) 이 사이클이 한 세션에서 끝나지 않고 다음 세션으로 이어지며 계속 쌓여갑니다 — 맥락 유지
...
기존 개발자 하네스 6축 평가는 균등 가중을 씁니다. 그러나 비개발자 하네스를 같은 방식으로 측정해보면 가중치가 비대칭으로 드러납니다.

- **맥락(입력) 약 50%** — "구조와 맥락이 좋으면 즉석 요청도 좋아진다"는 핵심 발견의 정량 표현입니다
- **구조(폴더·스킬·컨벤션) 약 15%** — 고정된 골격으로서 의미가 있으나 절대 비중은 아닙니다
- **출력(산출물 변환) 약 10%** — docx · pdf · 발표자료가 사람에게 전달되는 1차 수단입니다
- **맥락 유지(누적) 약 15%** — 다음 세션의 작동 전제가 이 축에서 옵니다
- **계획·실행·검증 합계 약 10%** — 즉석이 본질인 작업이라 이쪽 축은 약하게 작동합니다

이 가중치는 정설인 6축 균등 평균과 다른 약점·강점을 짚어내므로, 비개발자에게 더 정확한 진단을 내려줍니다.

## 고정된 핵심 패턴 3가지 — 3축에 정확히 매핑됩니다
...
이 가설은 의료기기 RA/QA + RWE 임상 QA 영역에서 비개발자(KHC RA팀장)가 Claude Code를 모노레포로 운용한 경험에서 나왔습니다. 핵심 발견은 다음 세 지점입니다.
